...
parser.add_argument("-p", 
                    "--path", 
                    help="The path of JSON.gz files to process")
parser.add_argument("-s",
                    "--sheet_name",
                    help="The corresponding event sheet name.")
parser.add_argument("-e",
                    "--event_id",
                    help="The event ID you are processing.")
...
def untuple_results_to_df(tweets: list) -> tuple:
    ...
    tweet_data["date"] = pd.to_datetime(tweet_data.created_at, unit='ms').dt.date
...
if __name__ == "__main__":
    files = glob.glob(os.path.join(args.path, "*.json.gz"))
    ...
    outfile.to_csv(os.path.join("data", "processed", args.sheet_name, args.event_id + "_update.csv"), index=False)
    author_data.to_csv(os.path.join("data", "processed", args.sheet_name, args.event_id + "_update_authors.csv"), index=False)
